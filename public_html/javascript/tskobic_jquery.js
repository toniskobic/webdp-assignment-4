...
function Cookie() {
  ...
  this.set = (username, type, name, surname) => {
    ...
    document.cookie =
      "username=" + username + "; expires=" + this.istice.toGMTString() + ";";
    document.cookie =
      "type=" + type + "; expires=" + this.istice.toGMTString() + ";";
    document.cookie =
      "name=" + name + "; expires=" + this.istice.toGMTString() + ";";
    document.cookie =
      "surname=" + surname + "; expires=" + this.istice.toGMTString() + ";";
  };
    ...
    $.ajax({
      ...
      success: function (json) {
        const array = json.filter(
          (element) => element.korisnicko_ime === login
        );
        if (array.length === 0) {
          return false;
        }
...
$(document).ready(function () {
  ...
  if ($(document).find("title").text() === "Galerija") {
    const endpoint =
      "http://barka.foi.hr/WebDiP/2020/materijali/zadace/dz3/userNameSurname.php?";
  ...
  if ($(document).find("title").text() === "Registracija") {
    ...
    $(".datetime").keyup(function () {
      ...
      let re1 = new RegExp(
        /^(19|[2-9][0-9])\d{2}\/([0-2]\d|3[0-1]|[1-9])\/(0?[1-9]|1[0-2])\s(1[0-2]|0?[1-9]):([1-5][0-9]|0?[1-9])\s(AM|PM)$/
      );
      let re2 = new RegExp(
        /^([0-2]\d|3[0-1]|[1-9])\.(0?[1-9]|1[0-2])\.(19|[2-9][0-9])\d{2}\.\s(0?[1-9]|1\d|2[0-3]):([1-5][0-9]|0?[1-9])$/
      );
